...
meshFace::meshFace(size_t v1, size_t v2, size_t v3)
    : a(v1), b(v2), c(v3)
{
}

meshFace::meshFace(size_t const indices[3]) : meshFace(indices[0], indices[1], indices[2])
{
}
...
bool meshFace::is_degenerate() const
{
    return a == b || b == c || c == a;
}
...
void mesh::computeNormals()
{
    ...
    for (size_t vi = 0; vi < mVertices.size(); vi++)
    {
        ...
        mVertexNormals[vi].set(vec3::average(faceNormals.cbegin(), faceNormals.cend()).unit());
    }
}
...
void mesh::faceClosestPt(size_t faceIndex, const vec3 &pt, vec3 &closePt, double &bestSqDist) const
{
...
mesh::mesh(const mesh &other) : mesh(other.mVertices.data(), other.num_vertices(), other.mFaces.data(), other.numFaces())
{
}

mesh::mesh(const vec3 *verts, size_t nVerts, const meshFace *faces, size_t nFaces)
...
double mesh::faceArea(size_t fi) const
{
...
bool mesh::contains(const vec3 &pt) const
{
    ...
    for (size_t fi : faces)
    {
        ...
        if (!utils::barycentricWithinBounds(bary))
            continue;
        hits.emplace_back(face_normal(fi).z, utils::barycentricEvaluate(bary, pts3).z);
    }
...
faceEdges::faceEdges(size_t p, size_t q, size_t r)
    : a(p), b(q), c(r)
{
}
