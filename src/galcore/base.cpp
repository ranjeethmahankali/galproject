...
vec3::vec3() : vec3(DBL_MAX_VAL, DBL_MAX_VAL, DBL_MAX_VAL) {}
...
vec3 vec3::operator-() const { return vec3(-x, -y, -x); }

double vec3::len_sq() const { return x * x + y * y + z * z; }

double vec3::len() const { return std::sqrt(len_sq()); }
...
bool vec3::is_zero() const { return x == 0 && y == 0 && z == 0; }
...
void vec3::set(const vec3 &v) { set(v.x, v.y, v.z); }
...
box3::box3(const vec3 &pt) : min(pt), max(pt) {}

box3::box3(const vec3 *points, size_t nPoints) : box3() {
  for (size_t i = 0; i < nPoints; i++) {
    inflate(points[i]);
  }
}

vec3 box3::diagonal() const { return max - min; }
...
void box3::deflate(double d) { inflate(-d); }
...
vec3 box3::center() const { return (min + max) * 0.5; }
...
double vec2::operator*(const vec2 &v) const { return x * v.x + y * v.y; }
...
bool vec2::operator!=(const vec2 &v) const { return v.x != x || v.y != y; }
...
double vec2::len_sq() const { return x * x + y * y; }

double vec2::len() const { return sqrt(len_sq()); }
...
bool vec2::is_zero() const { return x == 0.0 && y == 0.0; }

bool vec2::is_valid() const { return *this != vec2::unset; }

vec2 vec2::unit() const { return is_zero() ? vec2::zero : vec2(x, y) / len(); }
...
void vec2::set(const vec2 &v) { set(v.x, v.y); }
...
box2::box2(const vec2 &pt) : min(pt), max(pt) {}
...
box2::box2(const vec2 *points, size_t nPoints) : box2() {
  for (size_t i = 0; i < nPoints; i++) {
    inflate(points[i]);
  }
}

vec2 box2::diagonal() const { return max - min; }
...
void box2::deflate(double d) { inflate(-d); }
...
vec2 box2::center() const { return (min + max) * 0.5; }
...
vec3 utils::barycentricEvaluate(double const (&coords)[3],
                                vec3 const (&pts)[3]) {
