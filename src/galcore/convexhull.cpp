...
hullFace convexHull::popFace(size_t id, indexPair edges[3],
                             hullFace adjFaces[3]) {
  hullFace face;
  if (getFace(id, face)) {
    mFaces.erase(id);
...
double convexHull::facePlaneDistance(const hullFace &face,
                                     const vec3 &pt) const {
  return (pt - mPts[face.a]) * face.normal;
}

bool convexHull::getFarthestPt(const hullFace &face, vec3 &pt,
...
void convexHull::updateExteriorPt(const std::vector<hullFace> &newFaces,
