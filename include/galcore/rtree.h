...
template <class boost_point_t, typename vec_t, typename box_t> class rtree {
public:
  typedef boost_point_t point_type;
  typedef bgm::box<boost_point_t> box_type;
  typedef std::pair<box_type, size_t> item_type;
  typedef bgi::rtree<item_type, bgi::quadratic<RTREE_NUM_ELEMENTS_PER_NODE>>
      boost_tree_type;
  ...
  void query_by_distance(const vec_t &pt, double distance,
                         size_t_iter inserter) const {
    point_type center = to_boost(pt);
    query(bgi::satisfies([=](const item_type &item) {
...
template <> rtree2d::point_type rtree2d::to_boost(const vec2 &);

template <> vec2 rtree2d::from_boost(const point_type &);

template <> rtree3d::point_type rtree3d::to_boost(const vec3 &);

template <> vec3 rtree3d::from_boost(const point_type &);