...
typedef indexPaidHash edge_type_hash;
...
private:
    ...
    void faceClosestPt(size_t faceIndex, const vec3 &pt, vec3 &closePt, double &bestSqDist) const;
...
public:
    ...
    void query_box(const box3 &box, size_t_inserter inserter, meshElement element) const
    {
        element_tree(element).query_box_intersects(box, inserter);
    };

    template <typename size_t_inserter>
    void query_sphere(const vec3 &center, double radius, size_t_inserter inserter, meshElement element) const
    {
