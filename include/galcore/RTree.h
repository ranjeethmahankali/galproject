...
template <class BoostPointT, typename VecT, typename BoxT> class RTree {
public:
  typedef BoostPointT PointType;
  typedef bgm::box<BoostPointT> BoxType;
  typedef std::pair<BoxType, size_t> ItemType;
  typedef bgi::rtree<ItemType, bgi::quadratic<RTREE_NUM_ELEMENTS_PER_NODE>>
      BoostTreeType;

  void insert(const BoxT &b, size_t i) {
    mTree.insert(std::make_pair(toBoost(b), i));
  };
  ...
  template <typename SizeTIter>
  void queryNearestN(const VecT &pt, size_t numResults,
                       SizeTIter inserter) const {
