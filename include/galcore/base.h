...
// The number with the smallest absolute value that can be represented by the
// double datatype.
...
struct vec3 {
  ...
  static vec3 weighted_average(vec3_iter vFirst, vec3_iter vLast,
                               double_iter wFirst, double_iter wLast) {
    vec3 sum = vec3::zero;
    double wSum = 0;
...
void barycentricCoords(vtype const (&tri)[3], const vtype &pt,
                       double (&coords)[3]) {
  vtype v0 = tri[1] - tri[0], v1 = tri[2] - tri[0], v2 = pt - tri[0];
  double d00 = v0 * v0, d01 = v0 * v1, d11 = v1 * v1, d20 = v2 * v0,
         d21 = v2 * v1;
