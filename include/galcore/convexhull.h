...
private:
    ...
    std::unordered_map<size_t, hullFace, customSizeTHash, std::equal_to<size_t>> mFaces;
    std::unordered_map<indexPair, indexPair, indexPaidHash, std::equal_to<indexPair>> mEdgeFaceMap;
    ...
    void updateExteriorPt(const std::vector<hullFace> &newFaces, const std::vector<hullFace> &poppedFaces);
