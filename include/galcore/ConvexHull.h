...
static constexpr double PLANE_DIST_TOL = 1e-10;
...
private:
  ...
  std::unordered_map<size_t, Face, CustomSizeTHash, std::equal_to<size_t>>
      mFaces;
  ...
  bool faceVisible(const Face &, const vec3 &) const;
  double facePlaneDistance(const Face &, const vec3 &) const;
  bool getFarthestPt(const Face &, vec3 &pt, size_t &ptIndex) const;
  ...
  vec3 faceCenter(const Face &face) const;
